...
class BaseClient:
    ...
    async def _request(self, method: str, path: str, **kwargs):
        ...
        logger.info(f"--> [OUTGOING] {method} {url} | Headers: {headers} | Body: {body}")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                logger.info(f"<-- [INCOMING] {method} {url} | Status: {response.status_code} | Body: {response.text[:200]}")
                
                return response
            except httpx.RequestError as exc:
                logger.error(f"!!! [ERROR] {method} {url} | Exception: {str(exc)}")
                raise HTTPException(status_code=503, detail=f"Service at {self.base_url} is unavailable")

class FlightClient(BaseClient):
    async def get_flights(self, page: int, size: int):
        return await self._request("GET", "/api/v1/flights", params={"page": page, "size": size})

    async def get_flight(self, flight_number: str):
        return await self._request("GET", f"/api/v1/flights/{flight_number}")
...
class BonusClient(BaseClient):
    async def get_privilege(self, username: str):
        return await self._request("GET", "/api/v1/privilege", headers={"X-User-Name": username})

    async def calculate(self, username: str, payload: dict):
        return await self._request("POST", "/api/v1/privilege/calculate", 
                                  json=payload, 
                                  headers={"X-User-Name": username})

    async def rollback(self, username: str, ticket_uid: str):
        return await self._request("DELETE", f"/api/v1/privilege/rollback/{ticket_uid}", 
                                  headers={"X-User-Name": username})